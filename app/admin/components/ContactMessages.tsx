...
export const ContactMessages: React.FC<ContactMessagesProps> = ({
  ...
  const fetchContactData = async (isInitial = false) => {
    try {
      // Fetch stored contact queries and company contact info exclusively via Server Actions
      ...
      if (
        ...
        Array.isArray(actionMsgsRes.value.data)
      ) {
        setMessagesList(actionMsgsRes.value.data as ContactMessage[]);
      } else if (isInitial) {
      ...
      if (
        actionInfoRes.status === "fulfilled" &&
        actionInfoRes.value.success &&
        actionInfoRes.value.data
      ) {
      ...
      console.error("Error loading contact messages via Server Action:", err);
      if (isInitial && initialMessages.length > 0) {
        setMessagesList(initialMessages);
      }
    } finally {
      if (isInitial) {
  ...
  useEffect(() => {
    ...
    // Silent background refresh for dynamic frontend inquiries every 12 seconds
  ...
  useEffect(() => {
    if (messagesList.length > 0) {
      if (
        !activeMessageId ||
        !messagesList.some((m) => m.id === activeMessageId)
      ) {
  ...
  const activeMessage =
    messagesList.find((m) => m.id === activeMessageId) || filteredMessages[0];
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      if (res.success) {
        showToast(
          "success",
          res.message || "Client inquiry deleted successfully.",
        );
      } else {
        showToast("success", "Client inquiry deleted.");
      }
  ...
  const handleUpdateContactInfo = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        showToast(
          "success",
          res.message || "Company Contact Info updated successfully!",
        );
        setIsInfoModalOpen(false);
      } else {
        showToast("error", res.message || "Failed to update contact info.");
      }
    } catch (err: any) {
      showToast(
        "error",
        err?.message || "Failed to update contact info via Server Action.",
      );
    } finally {
      setSavingInfo(false);
    }
    ...
    <div className="space-y-6">
      ...
      >
        <div>
          ...
          <p
            className={`text-xs font-medium mt-1 ${darkMode ? "text-slate-400" : "text-slate-500"}`}
          >
            All user inquiries submitted directly from the contact page stored
            in database.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            type="button"
            onClick={() => fetchContactData(false)}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          ...
          >
            {isContactInfoLoading ? "Loading..." : phone || "Not Provided"}
          </p>
        </div>
        ...
        >
          ...
          <p className="text-sm font-black text-orange-500 truncate">
            {isContactInfoLoading ? "Loading..." : email || "Not Provided"}
          </p>
        </div>
        ...
        >
          ...
          <p
            className={`text-sm font-black truncate ${darkMode ? "text-white" : "text-slate-900"}`}
          >
            {isContactInfoLoading ? "Loading..." : fax || "Not Provided"}
          </p>
        </div>
        ...
        >
          ...
          <p
            className={`text-xs font-bold truncate ${darkMode ? "text-slate-300" : "text-slate-700"}`}
            title={address}
          >
            {isContactInfoLoading ? "Loading..." : address || "Not Provided"}
          </p>
        </div>
      </div>
              ...
              {paginatedMessages.length === 0 ? (
                <div className="text-center py-16 p-6 text-slate-400 space-y-2">
                  ...
                  <p className="text-xs font-bold text-slate-300">
                    No user queries matching filter.
                  </p>
                  <p className="text-[11px] text-slate-500">
                    Submissions from contact page will stream here
                    automatically.
                  </p>
                </div>
              ) : (
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span
                            className={`font-black text-sm ${darkMode ? "text-white" : "text-slate-900"}`}
                          >
                            {msg.name}
                          </span>
                          {msg.status === "unread" && (
                      ...
                      <div
                        className={`text-xs line-clamp-1 font-medium ${darkMode ? "text-slate-400" : "text-slate-600"}`}
                      >
                        {msg.message}
                      </div>
            ...
            >
              <div className="flex items-center gap-2">
                <span>
                  ...
                  of <strong className="text-orange-500">{totalItems}</strong>
                </span>
          ...
          {activeMessage ? (
              ...
              <div className="space-y-6">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="text-[11px] text-slate-400 font-mono flex items-center gap-1">
                          ...
                          Submitted: {activeMessage.createdAt}
                        </span>
                      </div>
                    </div>
                ...
                >
                  <h4 className="text-xs font-extrabold uppercase tracking-wider text-slate-400 mb-3 flex items-center gap-1.5">
                    <Inbox className="w-4 h-4 text-orange-500" /> Submitted
                    Inquiry Message
                  </h4>
            ...
            <div className="text-center py-24 text-slate-400 space-y-2">
              ...
              <p className="text-xs font-medium">
                Select a user query to view complete details.
              </p>
            </div>
          )}
        </div>
